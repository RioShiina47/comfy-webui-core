...
def process_krea2_controlnet_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    controlnets = []
    cn_images = all_ui_values.get(key('krea2_controlnet_images'), [])
    if not cn_images:
        return []

    cn_strengths = all_ui_values.get(key('krea2_controlnet_strengths'), [])
    cn_filepaths = all_ui_values.get(key('krea2_controlnet_filepaths'), [])
...
def process_hidream_o1_reference_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('hidream_o1_reference_images'), [])
    if not ref_images:
        return []
...
def process_reference_image_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('reference_image_images'), [])
    if not ref_images:
        return []
...
def process_boogu_image_edit_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('boogu_image_edit_images'), [])
    if not ref_images:
        return []
...
def process_qwen_image_edit_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('qwen_image_edit_images'), [])
    if not ref_images:
        return []
...
def process_krea2_identity_edit_inputs(all_ui_values: dict, prefix: str):
    key = lambda name: f"{prefix}_{name}"
    references = []
    ref_images = all_ui_values.get(key('krea2_identity_edit_images'), [])
    if not ref_images:
        return []
