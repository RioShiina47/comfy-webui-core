def inject(assembler, chain_definition, chain_items):
    ...
    ksampler_names = chain_definition.get('ksampler_node', 'ksampler')
    if not isinstance(ksampler_names, list):
        ksampler_names = [ksampler_names]

    nodes_injected = 0
    for ksampler_name in ksampler_names:
        ...
        print(f"EasyCache injector applied. KSampler '{ksampler_name}' model input re-routed through EasyCache node.")
        nodes_injected += 1

    if nodes_injected > 0:
        print(f"EasyCache injector finished. Applied to {nodes_injected} KSampler node(s).")